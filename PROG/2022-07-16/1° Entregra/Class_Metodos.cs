...
namespace _1__Entregra
{
    internal class Class_Metodos
    {
        ...
        public static void AbrirFormulario<MiForm>(Panel pnl) where MiForm : Form, new()
        {
            Class_Metodos.formulario = pnl.Controls.OfType<MiForm>().FirstOrDefault();
            if (formulario == null)
            {
        ...
        public static void Contraseña_Leave(ErrorProvider err, PictureBox pic, TextBox txt)
        {
            ...
            {
                err.SetError(pic, "Debe tener al menos 6 digitos, conteniendo letras, numeros y simbolos.");
            }
            else
            {
                err.SetError(pic, "");
        ...
        public static void PicBorrar_Light(TextBox txt)
        {
            txt.Text = "";
            txt.Focus();
        } 
        ...
        public static void PicBorrar_All(ErrorProvider err, PictureBox pic, TextBox txt1, TextBox txt2, TextBox txt3, TextBox txt4)
        {
            err.SetError(pic, "");
            txt1.Text = "";
            txt2.Text = "";
        ...
        {
            if (!Regex.IsMatch(txt.Text, "\\d") || !Regex.IsMatch(txt.Text, "[a-zA-ZÀ-ÿ\u00f1\u00d1]") || !Regex.IsMatch(txt.Text, "[^0-9a-zA-ZÀ-ÿ\u00f1\u00d1]") || !Regex.IsMatch(txt.Text, "\\S{6,20}"))
            {
                if (txt.Text.Equals(""))
        ...
        public static void Etapas(Button btn, Label lbl, ComboBox cob, String nombre, System.Drawing.Point lugar, bool visible)
        {
            btn.Text = nombre;
            btn.Location = lugar;
            lbl.Visible = visible;
        ...
        public static void Calculo_Fertilizante(NumericUpDown nud, double critico, double valorGe, string tipo, Label lbl)
        {
            if (Convert.ToDouble(nud.Value) == critico)
                lbl.Text = "No es necesario fertilizar.";
            else
            {
                MessageBox.Show(Convert.ToString(critico - Convert.ToDouble (nud.Value)+" X "+valorGe+" = "+ (critico - Convert.ToDouble(nud.Value)) * valorGe));
                lbl.Text=("Debe usar " + Convert.ToString((critico - Convert.ToDouble(nud.Value)) * valorGe) + "kg de " + tipo + ".");
            }
            lbl.Visible = true;
        }
    }
